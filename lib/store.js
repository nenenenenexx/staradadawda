...
export function validatePayment(session) {
  if (session.mode !== 'payment' || session.status !== 'complete' || !['paid', 'no_payment_required'].includes(session.payment_status) || !['usd', 'eur'].includes(session.currency)) {
    throw new Error('Payment does not match order');
  }
  // Use pre-discount amount so promo codes don't break plan matching.
  const plan = planForAmount(session.amount_subtotal ?? session.amount_total);
